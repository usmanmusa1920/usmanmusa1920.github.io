...
from account.default import blog_url_processed, general_context
from .forms import PostForm, CommentForm, CategoryForm, ReplyForm
from .models import Post, Comment, Reply, Category


def interate_mail(sender_name, sender_email, text_body, what, _url_):
    """ `what` should be either `comment` or `reply` """

    import os
    import kyaah

    subj = f'A {what} email sent through personal website - {sender_name}'
    body = f'Hi! Usman,\n\n{text_body}\n\nMail details:\n\tName: {sender_name}\n\tEmail: {sender_email}\n\t{what}_url: {_url_}'
    ...
    def post(request, post_id):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                h, u = request.headers['Origin'], request.path_info
                interate_mail(name, sender_email, text_body, what='comment', _url_=f'{h}{u}')
                
                instance.save()
                messages.success(request, f'Thanks {name} for your comment.')
    ...
    def comment(request, comment_id):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                h, u = request.headers['Origin'], request.path_info
                interate_mail(name, sender_email, text_body, what='reply', _url_=f'{h}{u}')

                instance.save()
                messages.success(request, f'Thanks {name} for your reply.')
